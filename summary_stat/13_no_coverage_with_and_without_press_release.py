...
if __name__ == "__main__":
    ...
    b = f'/ss/13_no_coverage_with_and_without_press_release/'
    save_dir = Constant.DROP_RES_DIR + b
    load_dir = data.p_dir+b
    ...
    if reload:
        ...
        press_release = data.get_press_release_bool_per_event(False)
        press_release['cik'] = press_release['cik'].astype(int)

        ev = ev.merge(press_release[['cik','form_id','release']],how='inner')

        # drop the event we never use
        ind = ev['items'].isin(Constant.LIST_ITEMS_TO_USE)
        ev = ev.loc[ind,:]

        # ev.groupby('items')['release'].mean()
        ...
        ev = ev.rename(columns={'items':'eightk','adate':'date'})
        ev['date'] = pd.to_datetime(ev['date'])

        press_release = ev.groupby(['date', 'permno'])['release'].max().reset_index()

        ev=ev.groupby(['date','permno'])['eightk'].count().reset_index()
        ...
        df_back_up = df.copy()
        df=df.dropna()
        df=df.merge(ev,how='left').merge(rav,how='left').merge(press_release,how='left')
        df =df.fillna(0.0)
        ...
        # for reload
        if with_correct_first_date:
            df.to_pickle(load_dir+'df_with_correct_first_date.p')
        else:
            df.to_pickle(load_dir+'df.p')
    else:
        if with_correct_first_date:
            df = pd.read_pickle(load_dir+'df.p')
        else:
            df = pd.read_pickle(load_dir+'df_with_correct_first_date.p')
    ...
    # run the main table
    for press_release in [0,1]:
        table = didi.TableReg()
        for y in ['abvol0','abs abret0']:
            for snp in [0,1]:
                ind = (df['in_snp'] == snp) & ((df['release']==press_release) | (df['eightk']==0))
        ...
        table.save_tex(save_dir=save_dir+f'main_reg_press{press_release}.tex')


    df.loc[df['eightk']==1,:].groupby(['release','in_snp'])['news0'].mean()
