...
if __name__ == "__main__":
    ...
    save_dir = Constant.DROP_RES_DIR + f'/ss/03_time_machine_bias/eventff{window}/'
    ...
    list_of_items = ['cat','items','items_name','sign','year']
    temp = df.loc[df['evttime']>=0,:].groupby(list_of_items+['uid'])['abret'].sum().reset_index()
    mean = temp.groupby(list_of_items)['abret'].mean().reset_index()
    count = temp.groupby(list_of_items)['abret'].count().reset_index()

    min_val = 100
    m = mean[count['abret']>=min_val].reset_index()
    count = count[count['abret']>=min_val].reset_index()


    count_for_attila = count.groupby(['items','sign'])['abret'].sum()
    count_for_attila=count_for_attila.reset_index().pivot(columns='sign',index='items',values='abret').fillna(f'<{min_val}')
    ...
    for cat in range(1, 10):
        ...
        for idx, pos_index in enumerate([1, 0]):
            temp = m.loc[m['sign'] == pos_index, :].pivot(columns='items', values='abret', index='year')
            temp = temp.dropna(axis=1, how='any')
            temp = temp[[x for x in temp.columns if np.floor(x) == cat]]
            temp.columns = [str(x)+ ' : ' +str(count_for_attila.loc[x,pos_index]/1e3) for x in temp.columns]
